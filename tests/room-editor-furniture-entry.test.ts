...
test('selected furniture area reopens the catalog instead of triggering a direct file upload', async () => {
  ...
  assert.equal(
    ...
    true,
    'RoomEditor should expose a catalog-opening action for adding more furniture'
  );

  assert.equal(
    source.includes('继续选择'),
    true,
    'RoomEditor should label the second furniture entry as a catalog selection action'
  );

  assert.doesNotMatch(
    source,
    /htmlFor=\{furnitureUploadInputId\}[\s\S]{0,200}继续添加/,
    'RoomEditor should not wire the second furniture entry directly to the hidden upload input'
  );
});
