...
npm test                                        # 全量 174 个用例，纯单元不连库不连网
node --test tests/*.test.ts tests/*.test.mjs    # 与 npm test 等价
node --test tests/generation-concurrency.test.ts  # 跑单个文件
```

CI：`.github/workflows/ci.yml` 在 push / PR 跑 `npm ci`、`npx tsc --noEmit`、`npm run lint`、`npm test`。
