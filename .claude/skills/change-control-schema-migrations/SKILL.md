...
- 查询构造集中在 `lib/server/generation-history-schema.ts`；**写路径固定使用 modern 多家具列**。legacy 单家具列只保留读兼容，不再保留 legacy 写入构造器。
...
- **脚本 8 个**：`migrate-better-auth.mjs`、`migrate-storage-assets.mjs`（只管数据库表/索引；R2 桶不在这里管理）、`migrate-vip-system.mjs`、`migrate-invite-system.mjs`、`migrate-generation-telemetry.mjs`、`set-admin.mjs`、`start-next.mjs`、`start-next-assets.mjs`（后两个是启动脚本不是迁移）。
