...
3. **公开白名单（仅四类，新增即架构决定）**：`/api/auth/[...all]`（better-auth 接管）、`POST /api/invitations/signup`（唯一公开注册入口 → [debug-auth-and-invites]）、`/api/healthz`、`/i/[code]`（GET 只校验/展示确认或跳注册；POST 才显式 claim 邀请）。
