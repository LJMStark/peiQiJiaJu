...
2. **【工具链】**：`eslint-config-next@16` vs `next@15` 版本错位仍保留，因为 `eslint-config-next@15.4.9` 在当前 ESLint 9 + flat config 下会让 `npm run lint` 启动失败；`package.json` 缺 `"type": "module"`（测试有性能警告）；**无任何 CI**。
