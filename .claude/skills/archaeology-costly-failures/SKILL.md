...
- **遗迹**（2026-07 仍在树上）：`lib/supabase-admin.ts`（死模块）、`migrate-storage-assets.mjs` 里的 Supabase 桶段落、`docs/plans/2026-03-14-supabase-storage*` 两份作废设计文档、`@supabase/supabase-js` 依赖。`19fd7ec` 曾附带 `migrate-storage-to-r2.mjs`，后被清理提交删除——**任何文档引用该脚本都是过时信号**。
